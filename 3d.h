...
//void   MultiplyMatrixByVector(double *resultvector, const double *matrix, const double *pvector);
...
inline double getWtime(struct timeval start, struct timeval end) 
{
  return (end.tv_sec-start.tv_sec+(end.tv_usec - start.tv_usec)*1.e-6);
}
double getTime();
